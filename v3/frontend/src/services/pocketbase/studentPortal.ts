...
export async function getMyStudentProfile(): Promise<StudentProfileRecord | null> {
  ...
  try { return await pb.collection(collections.studentProfiles).getFirstListItem<StudentProfileRecord>(`user = "${quote(user.id)}"`) }
  catch (error: unknown) { const status = typeof error === 'object' && error && 'status' in error ? Number((error as { status?: unknown }).status) : 0; if (status === 404) return null; throw error }
}
...
export async function listMyUpcomingClasses(limit = 10): Promise<ClassRecord[]> {
  requireStudentUser(); const now = new Date().toISOString()
