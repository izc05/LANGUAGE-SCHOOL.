...
export async function getMyStudentProfile(): Promise<StudentProfileRecord | null> {
  ...
  try {
    return await pb.collection(collections.studentProfiles).getFirstListItem<StudentProfileRecord>(`user = "${quote(user.id)}"`)
  } catch (error: unknown) {
...
export async function listMyEnrollments(): Promise<EnrollmentRecord[]> {
  const user = requireStudentUser()
  return pb.collection(collections.enrollments).getFullList<EnrollmentRecord>({ filter: `student = "${quote(user.id)}"`, sort: '-joined_at', expand: 'group,group.course' })
}

export async function listMyUpcomingClasses(limit = 10): Promise<ClassRecord[]> {
  requireStudentUser()
  const now = new Date().toISOString()
  const result = await pb.collection(collections.classes).getList<ClassRecord>(1, limit, { filter: `status = "SCHEDULED" && starts_at >= "${quote(now)}"`, sort: 'starts_at', expand: 'group,group.course' })
  return result.items
}
...
export async function listMyAttendance(limit = 40): Promise<AttendanceRecord[]> {
  const user = requireStudentUser()
  const result = await pb.collection(collections.attendance).getList<AttendanceRecord>(1, limit, { filter: `student = "${quote(user.id)}"`, sort: '-created' })
  return result.items
}
...
export async function listMyAssignments(limit = 30): Promise<AssignmentRecord[]> {
  requireStudentUser()
  const result = await pb.collection(collections.assignments).getList<AssignmentRecord>(1, limit, { filter: 'status != "DRAFT"', sort: 'due_at,-created' })
  return result.items
}

export async function listMySubmissions(limit = 50): Promise<SubmissionRecord[]> {
  const user = requireStudentUser()
  const result = await pb.collection(collections.assignmentSubmissions).getList<SubmissionRecord>(1, limit, { filter: `student = "${quote(user.id)}"`, sort: '-submitted_at' })
  return result.items
}
...
export async function getStudentDashboardSnapshot() {
  ...
  const [profile, enrollments, upcomingClasses, recentClasses, files, materials, assignments, submissions, notifications] = await Promise.all([
    getMyStudentProfile(), listMyEnrollments(), listMyUpcomingClasses(6), listMyRecentClasses(20), listMyFiles(8), listMyMaterials(8), listMyAssignments(12), listMySubmissions(30), listMyNotifications(10),
  ])
