...
export default function AdminCoursesPage() {
  ...
  async function handleCourseCover(course: CourseRecord, event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const data = new FormData(); data.set('cover_image', file)
      const updated = await pb.collection(course.collectionName || 'courses').update<CourseRecord>(course.id, data)
      setCourses((current) => current.map((item) => item.id === updated.id ? updated : item))
      setMessage(`Portada pública de ${course.title} actualizada correctamente.`)
  ...
  async function handleCreateCourse(event: FormEvent<HTMLFormElement>) {
    ...
    if (isDemoMode) { setMessage('Curso preparado en la demostración. No se ha guardado ningún cambio real.'); return }
    ...
    try {
      ...
      setCourses((current) => [...current, record].sort((a, b) => a.title.localeCompare(b.title, 'es'))); setGroupCourseId((value) => value || record.id)
      setCourseTitle(''); setCourseSlug(''); setCourseLevel(''); setCourseDescription(''); setCoursePublic(true); setMode(null); setMessage('Curso creado correctamente.')
    } catch (creationError) { setError(creationError instanceof Error ? creationError.message : 'No se ha podido crear el curso.') }
    finally { setSaving(false) }
  }

  async function handleCreateGroup(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(null); setMessage(null)
    if (isDemoMode) { setMessage('Grupo preparado en la demostración. No se ha guardado ningún cambio real.'); return }
    if (!groupCourseId || !groupTeacherId) { setError(activeCourses.length === 0 || activeTeachers.length === 0 ? 'Necesitas al menos un curso activo y un profesor activo antes de crear un grupo.' : 'Selecciona curso y profesor.'); return }
    setSaving(true)
    try { const record = await createAdminGroup({ name: groupName, courseId: groupCourseId, teacherId: groupTeacherId, academicYear, scheduleText, capacity, status: 'ACTIVE' }); setGroups((current) => [...current, record].sort((a, b) => a.name.localeCompare(b.name, 'es'))); setEnrollmentGroupId((value) => value || record.id); setSelectedGroupId(record.id); setGroupName(''); setScheduleText(''); setMode(null); setMessage('Grupo creado correctamente.') }
    catch { setError('No se ha podido crear el grupo.') } finally { setSaving(false) }
  }

  async function handleEnrollment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(null); setMessage(null)
    if (isDemoMode) { setMessage('Matrícula preparada en la demostración. No se ha guardado ningún cambio real.'); return }
    if (!enrollmentStudentId || !enrollmentGroupId) { setError('Selecciona alumno y grupo.'); return }
    setSaving(true)
    try { const record = await createAdminEnrollment({ studentId: enrollmentStudentId, groupId: enrollmentGroupId }); setEnrollments((current) => [record, ...current]); setMode(null); setMessage('Alumno matriculado correctamente.') }
    catch { setError('No se ha podido crear la matrícula.') } finally { setSaving(false) }
  }
  ...
  function openGroupEdit() { if (!selectedGroup) return; setEditGroupName(selectedGroup.name); setEditGroupTeacherId(selectedGroup.teacher); setEditAcademicYear(selectedGroup.academic_year); setEditScheduleText(selectedGroup.schedule_text || ''); setEditCapacity(selectedGroup.capacity); setShowGroupEdit(true); setError(null); setMessage(null) }
  async function saveGroupEdit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedGroup) return; const activeCount = activeCountForGroup(selectedGroup.id); const teacher = activeTeachers.find((item) => item.id === editGroupTeacherId); if (!editGroupName.trim() || !teacher || !editAcademicYear.trim()) { setError('Completa el nombre, el profesor activo y el curso académico.'); return }; if (!Number.isInteger(editCapacity) || editCapacity < activeCount || editCapacity < 1) { setError(`La capacidad no puede ser inferior a las ${activeCount} matrículas activas del grupo.`); return }; if (!window.confirm(`¿Guardar los cambios del grupo "${selectedGroup.name}"? Las matrículas y las clases existentes se conservan.`)) return; if (isDemoMode) { setShowGroupEdit(false); setMessage('Cambios del grupo preparados en la demostración. No se ha guardado ningún cambio real.'); return }; setSaving(true); setError(null); try { const updated = await updateAdminGroup(selectedGroup, { name: editGroupName.trim(), teacher: teacher.id, academic_year: editAcademicYear.trim(), schedule_text: editScheduleText.trim(), capacity: editCapacity }); setGroups((current) => current.map((item) => item.id === updated.id ? updated : item)); setEnrollments((current) => current.map((item) => item.group === updated.id ? { ...item, expand: { ...item.expand, group: updated } } : item)); setShowGroupEdit(false); setMessage('Grupo actualizado. Las matrículas y las clases ya registradas se conservan.') } catch { setError('No se ha podido actualizar el grupo.') } finally { setSaving(false) } }
