...
export default function TeacherStudentsPage() {
  ...
  useEffect(() => {
    ...
    listMyTeacherEnrollments()
      ...
      .catch(() => { if (mounted) setError('No se han podido cargar tus alumnos autorizados.') })
  ...
  useEffect(() => {
    ...
    Promise.all([getMyAuthorizedStudentProfile(selectedId), listMyAuthorizedStudentFiles(selectedId, 50)])
      ...
      .catch(() => { if (mounted) setError('No se ha podido abrir la ficha. La relación académica puede haber cambiado.') })
  ...
  async function downloadFile(record: StudentFileRecord) {
    ...
    if (isDemoMode) { setMessage('La descarga protegida estará disponible con PocketBase conectado.'); return }
    try { window.open(await getAuthorizedStudentFileDownloadUrl(record), '_blank', 'noopener,noreferrer') }
    catch { setError('No se puede descargar este archivo. PocketBase no autoriza la relación actual.') }
  }
    ...
    <DashboardShell role="Profesor" name="Profesor" nav={[...teacherNav]}>
      <div className="dashboard-content teacher-portal-page">
        <header className="teacher-page-heading">
          ...
          <div className="private-space-badge"><strong>{students.length}</strong><span>alumnos autorizados</span></div>
        </header>
        {loading && <div className="cms-notice">Cargando alumnos…</div>}{message && <div className="cms-notice success-notice">{message}</div>}{error && <div className="cms-notice auth-error">{error}</div>}

        <div className="teacher-students-grid">
          <section className="panel teacher-student-list-panel">
            <div className="panel-heading"><div><span className="eyebrow">ALUMNOS</span><h3>Grupos propios</h3></div><input className="student-files-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar..." /></div>
            <div className="teacher-student-picker">
              ...
              {!loading && visibleStudents.length === 0 && <p className="muted">No hay alumnos que coincidan.</p>}
            </div>
          </section>

          <section className="panel teacher-student-detail">
            {!selectedStudent && <div className="student-empty-detail"><span>ALUMNOS</span><h3>Selecciona un alumno</h3><p>Verás sus datos académicos permitidos y archivos privados de solo lectura.</p></div>}
            {selectedStudent && <>
              <div className="teacher-student-profile"><span className="teacher-profile-avatar">{selectedStudent.name.charAt(0).toUpperCase()}</span><div><span className="eyebrow">ALUMNO AUTORIZADO</span><h3>{selectedStudent.name}</h3><p>{selectedStudent.email || 'Sin email visible'}{selectedStudent.phone ? ` · ${selectedStudent.phone}` : ''}</p></div><span className="status success">Activo</span></div>
