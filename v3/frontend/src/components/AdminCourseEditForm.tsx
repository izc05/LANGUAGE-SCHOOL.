  ...
  return (
    <form className="admin-course-edit-form" aria-label={`Editar curso ${course.title}`} onSubmit={handleSubmit}>
      ...
      <div className="admin-functional-edit-grid">
        ...
        <label className="field-stack"><span>Nivel visible del curso</span><input value={level} onChange={(event) => setLevel(event.target.value)} placeholder="A2 · B1" /></label>
      ...
      <small className="admin-functional-help">El texto “Nivel visible” sigue siendo editorial. La compatibilidad MCER es el dato estructurado que utiliza el test para recomendar programas. Los grupos, matrículas y clases continúan vinculados por el ID del curso.</small>
