...
test('15B.3D.1: cambio de profesor de Grupo/Aula requiere decisión explícita y conserva histórico', async ({ page }) => {
  ...
  try {
    const courseA = await api(page, '/api/collections/courses/records', 'POST', {
      title: `D1 Course A ${suffix}`, slug: `d1-course-a-${suffix}`, level: 'B1', description: '15B.3D.1 E2E', status: 'ACTIVE', public_visible: false,
    })
    ...
    const courseB = await api(page, '/api/collections/courses/records', 'POST', {
      title: `D1 Course B ${suffix}`, slug: `d1-course-b-${suffix}`, level: 'B2', description: '15B.3D.1 E2E alternate course', status: 'ACTIVE', public_visible: false,
    })
    ...
    const teacherA = await createPrivilegedUser<{ id: string }>(page.request, {
      email: `d1-teacher-a-${suffix}@example.com`, password: 'D1TeacherPass123!', name: 'D1', surname: 'Teacher A', role: 'TEACHER', status: 'ACTIVE', phone: '',
    })
    ...
    const teacherB = await createPrivilegedUser<{ id: string }>(page.request, {
      email: `d1-teacher-b-${suffix}@example.com`, password: 'D1TeacherPass123!', name: 'D1', surname: 'Teacher B', role: 'TEACHER', status: 'ACTIVE', phone: '',
    })
    ...
    const group = await api(page, '/api/collections/groups/records', 'POST', {
      name: `D1 Group ${suffix}`, course: courseA.body.id, teacher: teacherA.body.id, academic_year: '2026/27', schedule_text: 'Lunes y miércoles · 18:00', capacity: 8,
    ...
    async function createClass(label: string, startsAt: string, endsAt: string, status: 'SCHEDULED' | 'COMPLETED' | 'CANCELLED') {
      const created = await api(page, '/api/collections/classes/records', 'POST', {
        group: groupId, teacher: teacherB.body.id, starts_at: startsAt, ends_at: endsAt, topic: label, description: '15B.3D.1 E2E class',
        status, delivery_mode: 'ONLINE', location_text: '', online_join_url: '',
      })
    ...
    expect((await api(page, `/api/collections/groups/records/${groupId}`)).body.teacher).toBe(teacherA.body.id)

    const missingDecision = await api(page, `/api/language-school/admin/academic/groups/${groupId}/update`, 'POST', { patch: { teacher: teacherB.body.id } })
    ...
    const keepFutureOverrides = await api(page, `/api/language-school/admin/academic/groups/${groupId}/update`, 'POST', {
      patch: { teacher: teacherB.body.id }, reassignFutureScheduledClasses: false,
    })
    ...
    expect((await api(page, `/api/collections/classes/records/${scheduledA.id}`)).body.teacher).toBe(teacherA.body.id)

    const scheduledB = await api(page, '/api/collections/classes/records', 'POST', {
      group: groupId, teacher: teacherA.body.id, starts_at: future(4), ends_at: future(4.1), topic: 'Future scheduled B', description: '15B.3D.1 E2E inherited teacher',
    ...
    const modeOnly = await api(page, `/api/language-school/admin/academic/groups/${groupId}/update`, 'POST', { patch: { default_delivery_mode: 'HYBRID' } })
    expect(modeOnly.status, JSON.stringify(modeOnly.body)).toBe(200)
    expect((await api(page, `/api/collections/classes/records/${scheduledB.body.id}`)).body.delivery_mode).toBe(modeBefore)

    const reassignFuture = await api(page, `/api/language-school/admin/academic/groups/${groupId}/update`, 'POST', {
      patch: { teacher: teacherA.body.id }, reassignFutureScheduledClasses: true,
    })
    ...
    const [futureAAfter, futureBAfter, pastAfter, completedAfter, cancelledAfter] = await Promise.all([
      api(page, `/api/collections/classes/records/${scheduledA.id}`), api(page, `/api/collections/classes/records/${scheduledB.body.id}`),
      api(page, `/api/collections/classes/records/${scheduledPast.id}`), api(page, `/api/collections/classes/records/${completed.id}`),
    ...
    expect((await api(page, `/api/language-school/admin/academic/groups/${groupId}/update`, 'POST', { patch: { course: courseB.body.id } })).status).toBe(400)
    expect((await api(page, `/api/collections/users/records/${teacherB.body.id}`, 'PATCH', { status: 'INACTIVE' })).status).toBe(200)
    expect((await api(page, `/api/language-school/admin/academic/groups/${groupId}/update`, 'POST', {
      patch: { teacher: teacherB.body.id }, reassignFutureScheduledClasses: true,
    })).status).toBe(400)
    expect((await api(page, `/api/collections/users/records/${teacherB.body.id}`, 'PATCH', { status: 'ACTIVE' })).status).toBe(200)

    const emptyGroup = await api(page, '/api/collections/groups/records', 'POST', {
      name: `D1 Empty Group ${suffix}`, course: courseA.body.id, teacher: teacherA.body.id, academic_year: '2026/27', schedule_text: '', capacity: 4,
    ...
    const emptyCourseChange = await api(page, `/api/language-school/admin/academic/groups/${emptyGroup.body.id}/update`, 'POST', { patch: { course: courseB.body.id } })
