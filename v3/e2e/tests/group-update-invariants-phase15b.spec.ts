...
test('15B: Grupo/Aula mantiene capacidad y dependencias activas en cualquier PATCH', async ({ page }) => {
  ...
  try {
    const course = await api(page, '/api/collections/courses/records', 'POST', {
      title: `Invariant Course ${suffix}`, slug: `invariant-course-${suffix}`, level: 'B1', description: 'Group invariant E2E', status: 'ACTIVE', public_visible: false,
    })
    ...
    const teacher = await createPrivilegedUser<{ id: string }>(page.request, {
      email: `group-invariant-teacher-${suffix}@example.com`, password: 'GroupInvariantTeacher123!',
    ...
    for (const index of [1, 2]) {
      const student = await createPrivilegedUser<{ id: string }>(page.request, {
        email: `group-invariant-student-${index}-${suffix}@example.com`, password: 'GroupInvariantStudent123!',
    ...
    const group = await api(page, '/api/collections/groups/records', 'POST', {
      name: `Invariant Group ${suffix}`, course: courseId, teacher: teacherId, academic_year: '2026/27', schedule_text: 'Martes y jueves · 18:00', capacity: 2,
    ...
    for (const studentId of students) {
      const enrollment = await api(page, '/api/collections/enrollments/records', 'POST', {
        student: studentId, group: groupId, status: 'ACTIVE', joined_at: new Date().toISOString(), ended_at: '',
      })
    ...
    expect((await api(page, `/api/collections/groups/records/${groupId}`, 'PATCH', { capacity: 1 })).status).toBe(400)
    expect((await api(page, `/api/collections/courses/records/${courseId}`, 'PATCH', { status: 'ARCHIVED' })).status).toBe(400)
    ...
    expect((await api(page, `/api/collections/courses/records/${courseId}`, 'PATCH', { status: 'ARCHIVED' })).status).toBe(200)
    expect((await api(page, `/api/collections/groups/records/${groupId}`, 'PATCH', { status: 'ACTIVE' })).status).toBe(400)
    expect((await api(page, `/api/collections/courses/records/${courseId}`, 'PATCH', { status: 'ACTIVE' })).status).toBe(200)

    expect((await api(page, `/api/collections/users/records/${teacherId}`, 'PATCH', { status: 'INACTIVE' })).status).toBe(200)
    expect((await api(page, `/api/collections/groups/records/${groupId}`, 'PATCH', { status: 'ACTIVE' })).status).toBe(400)
    expect((await api(page, `/api/collections/users/records/${teacherId}`, 'PATCH', { status: 'ACTIVE' })).status).toBe(200)
