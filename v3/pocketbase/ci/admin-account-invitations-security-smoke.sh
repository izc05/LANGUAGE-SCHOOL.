...
echo '1/14 Authenticate existing application ADMIN and CI superuser'
...
echo '2/14 Invite a new ADMIN without choosing their password or receiving the activation secret'
...
echo '3/14 ADMIN invitation creates no academic profile'
...
echo '4/14 INVITED ADMIN cannot authenticate with a guessed password'
...
echo '5/14 Resend rotates the ADMIN invitation while keeping its secret email-only'
...
echo '6/14 Invited ADMIN chooses their own password through the one-use secret'
...
echo '7/14 Activated ADMIN password alone receives MFA challenge, never a token'
...
echo '8/14 Existing ADMIN cannot replace another ADMIN password'
...
echo '9/14 Existing ADMIN cannot change another ADMIN verification credential'
VERIFIED_STATUS="$(curl -sS -o "$PATCH_BODY" -w '%{http_code}' -X PATCH "$PB_URL/api/collections/users/records/$ADMIN_ID" \
  -H "Authorization: $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  --data '{"verified":false}')"
test "$VERIFIED_STATUS" = '403'
echo 'OK: admin-to-admin verified mutation is blocked server-side'

echo '10/14 Existing ADMIN cannot redirect another ADMIN security email'
...
echo '11/14 Existing ADMIN cannot change another ADMIN lifecycle state via generic API'
STATE_STATUS="$(curl -sS -o "$PATCH_BODY" -w '%{http_code}' -X PATCH "$PB_URL/api/collections/users/records/$ADMIN_ID" \
  -H "Authorization: $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  --data '{"status":"SUSPENDED"}')"
test "$STATE_STATUS" = '403'
UNCHANGED_RECORD="$(curl -fsS "$PB_URL/api/collections/users/records/$ADMIN_ID" -H "Authorization: $SUPER_TOKEN")"
test "$(jq -r '.status' <<<"$UNCHANGED_RECORD")" = 'ACTIVE'
echo 'OK: admin-to-admin status mutation is blocked'

echo '12/14 Existing ADMIN cannot edit even non-security fields of another ADMIN through generic API'
PROFILE_STATUS="$(curl -sS -o "$PATCH_BODY" -w '%{http_code}' -X PATCH "$PB_URL/api/collections/users/records/$ADMIN_ID" \
  -H "Authorization: $ADMIN_TOKEN" -H 'Content-Type: application/json' \
  --data '{"name":"Forged name","phone":"699999999"}')"
test "$PROFILE_STATUS" = '403'
UNCHANGED_RECORD="$(curl -fsS "$PB_URL/api/collections/users/records/$ADMIN_ID" -H "Authorization: $SUPER_TOKEN")"
test "$(jq -r '.name' <<<"$UNCHANGED_RECORD")" = 'CI'
echo 'OK: generic ADMIN-to-ADMIN updates are fully closed'

echo '13/14 Existing ADMIN cannot delete another ADMIN via generic users API'
...
echo '14/14 Activation token remains one-use'
