...
routerAdd('POST', '/api/language-school/placement/attempts/{id}/answer', (e) => {
  ...
  const existing = attemptAnswers(e.app, attempt.id).find((answer) => answer.getString('question') === questionId)
  ...
  const answer = new Record(e.app.findCollectionByNameOrId('placement_answers'))
  answer.set('attempt', attempt.id)
  answer.set('question', question.id)
  answer.set('selected_option_id', optionId)
  answer.set('is_correct', optionId === question.getString('correct_option_id'))
  answer.set('points_awarded', optionId === question.getString('correct_option_id') ? 1 : 0)
  answer.set('answered_at', nowIso())
  e.app.save(answer)
...
routerAdd('POST', '/api/language-school/placement/attempts/{id}/finish', (e) => {
  ...
  const scoredItems = entries.map((entry) => {
    ...
    if (!answer) throw new InternalServerError('Falta una respuesta del intento.')
    return {
      skill: question.getString('skill'),
      level: question.getString('cefr_level'),
      correct: answer.getBool('is_correct'),
    }
  })
